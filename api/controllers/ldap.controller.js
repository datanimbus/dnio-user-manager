...
function getUsersFromAD(req, res) {
	...
	if (accToken) {
		const decrypted = decrypt(accToken);
		...
		res.status(404).json({ message: 'adToken not found' });
	}
	/*
...
function searchUsers(req, res) {
	...
	getConnectionDetails(req)
		.then(_cd => {
			connectionDetails = _cd;
			mapping = connectionDetails.mapping;
			let requiredMapping = ['name', 'username'];
			if (requiredMapping.some(_k => !mapping[_k] || typeof mapping[_k] !== 'string' || mapping[_k].length == 0)) {
				return res.status(400).json({ message: requiredMapping + 'is required' });
			}
			return ldapUtil.connectLDAP(_cd.url, _cd.bindDN, _cd.bindPassword);
		})
		.then(client => {
			let attributes = null;
			if (mapping) {
			...
			let filter = '';
			if (connectionDetails.baseFilter) {
				filter = '&' + connectionDetails.baseFilter;
			}
			if (strictSearch) {
			...
			return ldapUtil.searchLdap(client, connectionDetails.baseDN, filter, attributes);
		}, () => {
